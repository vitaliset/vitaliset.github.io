...
<p><div align="justify">$\oint$ <em>This inequality is rude because  ${A \in (m_a, M_a)}^C $ and $ { B \in (m_b, M<em>b)}^C$ have a significant intersection. However, we assume it's zero when we overestimate the probability of the union by the sum of the probabilities (we are presuming they are disjoint intervals)</em>.</div></p>
